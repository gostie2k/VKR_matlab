#!/usr/bin/env bash
# D6 — перепрогон V3 после трёх RTL-правок этапа 3.12 (post-OOC fixes):
#   1) sync_top.v:151        rst → reset (ambiguous clock fix)
#   2) sync_mod1_nco.v       mu_full / w_step → mu_simple битовый сдвиг
#   3) sync_loop_filter_pi.v + pipeline-ступень (+1 такт латентности)
#
...
p = "$STIM"
s = open(p).read()
s = re.sub(r'^EBN0_DB\s*=.*$', f'EBN0_DB     = ${EBNO}.0      # Eb/N0, дБ (D6)', s, count=1, flags=re.M)
open(p,'w').write(s)
PY
...
run_one 20 "$SIM/debug_v3/D6_post_synth_fix"
run_one 60 "$SIM/debug_v3/D6_post_synth_fix_ebno60"
...
open(p,'w').write(s)
PY
echo "[D6] done. EBN0_DB restored to 20.0"
