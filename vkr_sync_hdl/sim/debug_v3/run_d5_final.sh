#!/usr/bin/env bash
# D5 — финальный прогон V3 с D3-патчем в основной ветке RTL.
...
p = "$STIM"
s = open(p).read()
s = re.sub(r'^EBN0_DB\s*=.*$', f'EBN0_DB     = ${EBNO}.0      # Eb/N0, дБ (D5)', s, count=1, flags=re.M)
open(p,'w').write(s)
PY
    ...
    # Пересоздать проект (RTL изменился), затем запустить sim.
...
run_one 20 "$SIM/debug_v3/D5_final"
run_one 60 "$SIM/debug_v3/D5_final_ebno60"
...
open(p,'w').write(s)
PY
echo "[D5] done. EBN0_DB restored to 20.0"
