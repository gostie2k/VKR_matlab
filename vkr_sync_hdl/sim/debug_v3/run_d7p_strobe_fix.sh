#!/usr/bin/env bash
# D7p — перепрогон V3 после strobe-fix в sync_top.v (вариант A решения по D7):
#   sync_top.v: добавлен strobe_d3 для согласования с новой латентностью
#   Farrow = 4 такта. Pipeline-вставки итерации 3 (Farrow/TED/AGC) сохранены.
#
...
p = "$STIM"
s = open(p).read()
s = re.sub(r'^EBN0_DB\s*=.*$', f'EBN0_DB     = ${EBNO}.0      # Eb/N0, дБ (D7p)', s, count=1, flags=re.M)
open(p,'w').write(s)
PY
...
run_one 20 "$SIM/debug_v3/D7p_strobe_fix"
run_one 60 "$SIM/debug_v3/D7p_strobe_fix_ebno60"
...
open(p,'w').write(s)
PY
echo "[D7p] done. EBN0_DB restored to 20.0"
